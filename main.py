...
for filepath in filepaths:
    ...
    for i, key in enumerate(df.keys()):
        pdf.set_y(pdf_y)
        pdf.set_x(pdf_x)
        pdf.multi_cell(get_column_width(key), line_height, get_title(key), border=1)
    ...
    for i, row in df.iterrows():
        ...
        for key in row.keys():
            ...
            pdf_x = pdf_x + get_column_width(key)
            if key == 'total_price':
                total_price += row.get(key)

    # Create summary
    pdf.cell(w=0, h=12, txt="The total amount due is: " + str(total_price), align="L", ln=1, border=0)

    # Save file
    pdf_filepath = filename + '.pdf'
    pdf.output("pdfs/" + pdf_filepath)
